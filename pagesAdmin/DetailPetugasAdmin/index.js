...
export class DetailPetugasAdmin extends Component {
  constructor(props){
    super(props);
    this.state = {
      data: {},
  ...
  async getData(){
    ...
    await get(child(dbRef, `dataPetugas/${idPetugas}`)).then(async (snapshot) => {
      if (snapshot.exists()) {
        this.setState({
          data: snapshot.val(),
  ...
  onHapus(id){
    const dbRef = ref(getDatabase());
        ...
        remove(child(dbRef, `dataPetugas/${id}`))
        .then(() => {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.wrapperTombol}>
          <View style={styles.tombolCol1}>
            <TouchableOpacity style={styles.tombolSecondary} onPress={() => this.props.navigation.navigate('UbahPetugasAdmin', {
          ...
          <TouchableOpacity style={styles.tombolTersier} onPress={() => Alert.alert('Hapus Anggota', 'Yakin ingin menghapus anggota?', [{
                            ...
                            {
                                text: 'Hapus',
                                onPress: () => this.onHapus(this.state.idPetugas)
                            }
                            ]) }>
...
export default DetailPetugasAdmin