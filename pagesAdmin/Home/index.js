...
import { faInfo, faUser, faUsers, faUserPlus, faUserCheck, faUserEdit, faUserGear, faUserTie, faLightbulb, faUserTiePlus, faQuestionCircle, faCalculator, faSignOut} from '@fortawesome/free-solid-svg-icons'
...
export class HomeAnggota extends Component {
    ...
    async componentDidMount(){
        ...
        await get(child(dbRef, `dataAdmin/${userId}`)).then((snapshot) => {
        if (snapshot.exists()) {
            this.setState({
    ...
    periksaPermohonan(){
     ...
     async getPetugas(){
        const userId = await AsyncStorage.getItem('idUser')
        const dbRef = ref(getDatabase());
        get(child(dbRef, `dataAdmin/${userId}`)).then((snapshot) => {
        if (snapshot.exists()) {
            this.setState({
                wilayah: snapshot.val().wilayah,
            })
        ...
        <View style={styles.container}>
            ...
            <View style={styles.body}>
                    <View style={styles.bodyWrapperButton}>
                        <TouchableOpacity style={styles.bodyButton} onPress={()=> this.props.navigation.navigate('DaftarPetugasAdmin')}>
                            <FontAwesomeIcon icon={ faUsers } style={styles.bodyIcon} size={25}/>
                            <Text style={styles.bodyText}>Daftar Petugas</Text>
                        </TouchableOpacity>
                    </View>
                    <View style={styles.bodyWrapperButton}>
                        <TouchableOpacity style={styles.bodyButton} onPress={()=> this.props.navigation.navigate('TambahPetugasAdmin')}>
                        <FontAwesomeIcon icon={ faUserPlus } style={styles.bodyIcon} size={25}/>
                            <Text style={styles.bodyText}>Tambah Petugas</Text>
                        </TouchableOpacity>
                    </View>
                    {/* <View style={styles.bodyWrapperButton}>
                        <TouchableOpacity style={styles.bodyButton} onPress={()=> this.props.navigation.navigate('DaftarAdmin')}>
                        <FontAwesomeIcon icon={ faUserTie } style={styles.bodyIcon} size={25}/>
                            <Text style={styles.bodyText}>Daftar Admin</Text>
                        </TouchableOpacity>
                    </View> */}
            ...
            <View style={styles.body}>
                    {/* <View style={styles.bodyWrapperButton}>
                        <TouchableOpacity style={styles.bodyButton} onPress={()=> this.props.navigation.navigate('TambahAdmin')}>
                        <FontAwesomeIcon icon={ faUserPlus } style={styles.bodyIcon} size={25}/>
                            <Text style={styles.bodyText}>Tambah Admin</Text>
                        </TouchableOpacity>
                    </View> */}
