import { Text, View, StyleSheet, TextInput, TouchableOpacity, Picker, Alert, Image, ScrollView, FlatList } from 'react-native'
...
export class TambahAdmin extends Component {
  ...
  onChangeTanggal = (event, selectedDate) => {
    ...
    async onSubmit(){
      if (!this.state.email || !this.state.password || !this.state.nama || !this.state.nik || !this.state.tempatLahir || !this.state.tanggalLahir || !this.state.jenisKelamin ) {
      ...
      const db = getDatabase()
        ...
        onValue(refEmail, async (snapshot) => {
            ...
            if (this.state.isEmail) {
              ...
              const refAnggota = query(ref(db, "dataPetugas"), orderByChild("nik"), equalTo(this.state.nik))
              onValue(refAnggota, async (snapshot) => {
                  ...
                  const idUser = this.state.nama + tanggal
                  ...
                  if (this.state.foto == 'https://firebasestorage.googleapis.com/v0/b/stunt-5913f.appspot.com/o/fotoAnggota%2Fuser.png?alt=media&token=b162b3d4-3e41-4895-8922-6577ed6b4b9c') {
                    ...
                    const fotoAnggota = sRef(storage, "fotoAdmin/"+ this.state.nik + ".jpg")
                    ...
                    await set(ref(db, 'dataAdmin/' + idUser), {
                      id: idUser,
                    ...
                    this.props.navigation.navigate('DaftarAdmin')
                  }
                }, {
                  onlyOnce: true
              });
  ...
  render() {
    ...
    return (
      ...
      <View style={styles.container} >
        <Text style={styles.text1}>Tambah Admin</Text>
        ...
        <TouchableOpacity style={styles.tombolLogin} onPress={()=>{ this.onSubmit()}}>
          <Text style={styles.textTombolLogin}>Daftar</Text>
        </TouchableOpacity>
